...
# =======================================================
# 🏖️ LISTINO TARIFFE AGGIORNATO (FOGLIO NUOVO 2026)
# =======================================================
# Struttura dei dati: [Prezzo_Ombrellone_2Lettini, Supplemento_Persona_Extra]
TARIFFE = {
    "Media 1": {
    ...
    if raw_input:
        ...
        if "Sinistra" in modalita:
            ...
            nuova_p = pd.DataFrame([{
                ...
                "Persone": 2, "Durata": duration_assigned := durata_assegnata, "Extra": "",
...
if nome_wa and len(date_wa) > 0:
    ...
    stringa_date_ita = f"dal {d1} al {date_wa[1].strftime('%d/%m/%Y')}" if len(date_wa) > 1 and date_wa[0] != date_wa[1] else f"per il giorno {d1}"
    ...
    # (Altre lingue compilate speculari con la dicitura corretta e la variabile fila_formattata)
