...
CAPIENZA_FILE = {
    "Prima Fila": 17,
    "Seconda Fila": 17,
    "Terza Fila": 12, # Aggiornato a 12 ombrelloni totali
    "Quarta Fila": 10,
    "Quinta Fila": 7,
    "Sesta Fila (Altre)": 6
}
                    ...
                    for idx in edited_df.index:
                        try:
                            if idx in risultati.index:
                                old_fila = str(risultati.loc[idx, 'Fila'])
                                new_fila = str(edited_df.loc[idx, 'Fila'])
                                old_durata = str(risultati.loc[idx, 'Durata'])
...
input_nome = st.sidebar.text_input("👤 Nome e Cognome (Obbligatori per nuovi)", key=f"form_nome_{rk}").strip()
input_telefono = st.sidebar.text_input("📱 Telefono (Obbligatorio per nuovi futuri)", key=f"form_tel_{rk}").strip()
...
st.sidebar.markdown("---")
tipo_salvataggio = st.sidebar.radio("Se la postazione risulta già occupata:", ["Blocca (Errore)", "Sostituisci (Cancella il vecchio)", "Subentro (Tieni il vecchio per l'incasso)"], help="Scegli Subentro per far pagare due clienti sulloCampione dello stesso ombrellone nello stesso giorno")
                ...
                if input_telefono:
                    ...
                    if not df_clienti.empty:
                        ...
                        if tel_norm in df_clienti['Tel_Norm'].values:
                            ...
                            nuovo_c = pd.DataFrame([{"Telefono": input_telefono, "Nome": nome_da_salvare}])
                    ...
                    for j in range(quantita_postazioni):
                        ...
                        if stato_pulito != "Libero":
                            nuova_p = pd.DataFrame([{
                                ...
                                "Nome": nome_da_salvare, "Telefono": input_telefono, "Stato": stato_pulito,
    ...
    if giorni_totali_vis == 1:
        ...
        for nome_fila, max_posti in CAPIENZA_FILE.items():
            ...
            for i in range(max_posti):
                ...
                # --- CALCOLO NUOVO ORIENTAMENTO FISICO REALE SCALATO AGGIORNATO ---
                etichetta = ""
            ...
            for idx in edited_range.index:
                try:
                    if idx in df_range.index:
                        old_fila = str(df_range.loc[idx, 'Fila'])
                        new_fila = str(edited_range.loc[idx, 'Fila'])
                        old_durata = str(df_range.loc[idx, 'Durata'])
